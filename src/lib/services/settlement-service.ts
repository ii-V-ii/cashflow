...
export async function getMonthlySettlement(
  ...
  const [categoryTotals, accountChanges, previousMonth] = await Promise.all([
    getCategoryTotalsForMonth(start, end),
    getAccountChangesForMonth(year, month),
    getPreviousMonthTotals(year, month),
  ])
...
export async function getAnnualSettlement(
  ...
  const [months, categoryTotals, previousYear] = await Promise.all([
    getMonthlyTotalsForYear(year),
    getCategoryTotalsForYear(`${year}-01-01`, `${year + 1}-01-01`),
    getYearTotals(year - 1),
  ])
...
async function getAccountChangesForMonth(year: number, month: number): Promise<AccountChange[]> {
  ...
  // 3개 쿼리 병렬 실행 (allAccounts, preMonthEffects, monthEffects 독립적)
  const [allAccounts, preMonthEffects, monthEffects] = await Promise.all([
    ...
    `) as unknown as Promise<Array<{ account_id: string; net_effect: number }>>,
    ...
    db.execute(sql`
      ...
      GROUP BY e.account_id
    `) as unknown as Promise<Array<{ account_id: string; income: number; expense: number }>>,
  ])
